...
metadata {
		definition (name: "Tasmota Plug with Power Monitoring for ST Classic V0.93", namespace: "garyjmilne", author: "Gary J. Milne")  {
		capability "Switch"
	...
	tiles(scale: 2){
        ...
        standardTile("operatingState", "device.operatingState", inactiveLabel: false, decoration: "flat", width: 3, height: 2 ) {
        	state "off", label:"Device Off", action:"", defaultState: false, icon:"https://img.icons8.com/color/96/000000/tv-off.png" //"https://img.icons8.com/color/96/000000/light-off.png"
...
private log(name, message, loglevel){
    ...
    def filterlist = "listofnamestofilter: function1, anothername, etc."
...
private message(message){
    ...
    sendEvent(name:"Message", value: icon1 + message, isStateChange:true, displayed:false)    
}
...
def initialize(){
    ...
    if ( device.currentValue("operatingState") == null ) {
    	log.debug "operatingState: " + device.currentValue("operatingState")
    	//sendEvent(name: "operatingState", value: "off", isStateChange:true)
    }
...
def on() {
	...
	if ( isSystemIdle() == true ){	
        ...
        if (checkResponse() == true) {
        	...
        	if ( device.currentValue("switch") != "off" ) sendEvent(name:"switch", value:"off", isStateChange:true, displayed:false)
...
def updatePowerState(state){
	log ("updatePowerState", "Value:" + state, 2)
	switch(state) { 
		case ["off"]:                
            if (device.currentValue("switch") != "off") {
                ...
                sendEvent(name:"stateDuration", value:0, isStateChange:false, displayed:false)
...
def setOperatingState(){
    ...
    log("setOperatingState", "Current Watts are:(${Watts}) .... StandbyThreshold is: ${standbyThreshold}" + " ...Operating State is:" + device.currentValue("operatingState"), 2)
    if (Watts == 0 ) {
    	if (device.currentValue("operatingState") != "off"){
        	sendEvent(name:"operatingState", value: "off", isStateChange:true, displayed:true)
            }
	...
	sendEvent(name:"stateDuration", value:timeSince(since), isStateChange:true, displayed:false)
}
...
def plug(){
    ...
    if ( usingLocalTime() == true) {
    	...
    	sendEvent(name:"multi", value:"UTC", isStateChange:true, displayed:false)
        }
    else {
    	log ("plug", "usingLocalTime: false", 1)
    	sendEvent(name:"multi", value:"Local", isStateChange:true, displayed:false)
    }
}
	...
	if ( isSystemIdle() == true ){  //Performs a check to see if another command is in progress already 
        ...
        if ((device.currentValue("commandflag") == "Complete" ) || (elapsedTime() > 10)){
            ...
            if (checkResponse() == true) {
                ...
                mytime = elapsedTime()
                log ("sync", "Sync Success...${mytime} seconds", 1)
            ...
            sendEvent(name:"sync", value:"idle", isStateChange:true, displayed:false)
            }
	else{
		log ("Sync", "Not executed..command in progress: " + device.currentValue("commandflag"), 1)
		}
	}
    setOperatingState()
}
...
def isSystemIdle(){
    ...
    if ((device.currentValue("commandflag") == "Complete" ) || (elapsedTime() > 10)){
    	log ("isSystemIdle", "True - callTasmota() - Allowed", 1)
		return true
        }
   else {
   		log ("isSystemIdle", "False - callTasmota() - Blocked", 1)
   		message("System busy")
        return false
	}
}
...
def poll(){
	    log ("Poll", "Polling started.. ", 1)
        if (settings.frequency > 1) 
        	sendEvent(name:"sync", value:"running", isStateChange:true, displayed:true)
        else
        	sendEvent(name:"sync", value:"running", isStateChange:true, displayed:false)
        sync()
        if (settings.frequency > 1) 
...
def checkResponse(){
	...
	for (x = 10; x >= 0; x--) {
        ...
        else
        {
            ...
            if (device.currentValue("status") != "wait") sendEvent(name:"status", value: "wait", isStateChange:true, displayed:false )
        }
    }
    ...
    switch(device.currentValue("commandflag")) { 
        ...
        //Update the status to show we have received a response
        sendEvent(name:"status", value:"success", isStateChange:true, displayed:true )
        result = true
        break
        ...
        sendEvent(name:"status", value:"fail", isStateChange:true, displayed:true )
        result = false
        break;
    }
